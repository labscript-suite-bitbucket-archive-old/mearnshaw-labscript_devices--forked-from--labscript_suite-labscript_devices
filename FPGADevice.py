...
class FPGADevice(PseudoclockDevice):
    ...
    def digital_outputs(self):
        # for digital outputs we need to pretend the clock limit is
        # double what it is in reality because our clocks/toggles system
        # does not limit triggering to rising edges only.

        # FIXME: is this really the best way to handle this?
        self.clock_limit *= 2
        output = self.outputs()
        self.clock_limit /= 2
        return output
